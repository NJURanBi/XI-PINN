...
def optimize_parameters_Adam(data_op, data_on, data_b, data_ini, data_if, beta_p, beta_n, model, epochs, lr, lossval, lossval_dbg, relative_error, device):
    ...
    def loss_interface_N(data):
        ...
        un = model(data[0])
        n = data[2]
        gradup = autograd.grad(outputs=up, inputs=data[1], grad_outputs=torch.ones_like(up), create_graph=True,
                               retain_graph=True)
        ...
        res = (beta_p * dupdx - beta_n * dundx) * n[:, 0][:, None] + (beta_p * dupdy - beta_n * dundy) * n[:, 1][:,
                                                                                                         None]
    ...
    torch.save(model.state_dict(), 'best_model_PINN_Adam.mdl')
...
def optimize_parameters_SOAP(data_op, data_on, data_b, data_ini, data_if, beta_p, beta_n, model, epochs, lr, lossval, lossval_dbg, relative_error, device):
    ...
    def loss_interface_N(data):
        ...
        un = model(data[0])
        n = data[2]
        gradup = autograd.grad(outputs=up, inputs=data[1], grad_outputs=torch.ones_like(up), create_graph=True, retain_graph=True)
        gradun = autograd.grad(outputs=un, inputs=data[0], grad_outputs=torch.ones_like(un), create_graph=True, retain_graph=True)
        dupdx = gradup[0][:, 0][:, None]
        dupdy = gradup[0][:, 1][:, None]
        dundx = gradun[0][:, 0][:, None]
        dundy = gradun[0][:, 1][:, None]
        res = (beta_p * dupdx - beta_n * dundx) * n[:, 0][:, None] + (beta_p * dupdy - beta_n * dundy) * n[:, 1][:, None]
        loss = torch.mean(res**2)
        return loss
    ...
    for epoch in range(epochs + 1):
        ...
        if epoch % 100 == 0:
            print('epoch:', epoch, 'loss:', loss.item(), 'loss1:', loss1.item(), 'loss2:', loss2.item(), 'loss3:',
                  loss3.item(), 'loss4:', loss4.item(), 'loss5:', loss5.item())
            current_lr = optimizer.param_groups[0]['lr']
            print(f"Epoch {epoch}, Learning Rate: {current_lr:.2e}")
            func_params = dict(model.named_parameters())
            error = error_compute(model, func_params, data_test)
    ...
    print('best epoch:', best_epoch, 'best loss:', best_loss)
    model.load_state_dict(best_state_dict)
    torch.save(model.state_dict(), 'best_model_PINN_SOAP.mdl')
    return model, lossval, lossval_dbg, relative_error
...
def optimize_parameters_LM(data_op, data_on, data_b, data_ini, data_if, beta_p, beta_n, model, tr_iter_max, mu, lossval, lossval_dbg, relative_error, mu_div, mu_mul, device):
    ...
    def res_omega_p(func_params, data, f):
        ...
        d2udy2 = grad2u[1][1]
        f_val = f[0]
        res = dudt - beta_p * (d2udx2 + d2udy2) - f_val
        return res
    ...
    def res_omega_n(func_params, data, f):
        ...
        d2udy2 = grad2u[1][1]
        f_val = f[0]
        res = dudt - beta_n * (d2udx2 + d2udy2) - f_val
        return res
    ...
    def res_interface(func_params, data, data_eps, nor):
        grad_p = torch.func.jacrev(u_NN, argnums=0)(data_eps, func_params)
        grad_n = torch.func.jacrev(u_NN, argnums=0)(data, func_params)
        dupdx, dupdy = grad_p[0], grad_p[1]
        dundx, dundy = grad_n[0], grad_n[1]
        n1, n2 = nor[0], nor[1]
        res = (beta_p * dupdx - beta_n * dundx) * n1 + (beta_p * dupdy - beta_n * dundy) * n2
        return res
    ...
    try:
        while (lossval[-1] > tol_main) and (step <= tr_iter_max):
            ...
            if (Comput_old == True):  # need to compute loss_old and J_old
                ...
                Lif = torch.vmap((res_interface), (None, 0, 0, 0))(func_params, data_if[0], data_if[1], data_if[2]).flatten().detach()
            ...
            if criterion:
                ...
                per_sample_grads = torch.vmap(torch.func.jacrev(res_interface), (None, 0, 0, 0))(func_params, data_if[0], data_if[1], data_if[2])
                cnt = 0
                for g in per_sample_grads:
                    g = per_sample_grads[g].detach()
        ...
        print(f" training loss: {lossval[-1]:.4e}")
        print('finished')
        torch.save(model.state_dict(), 'best_model_PINN_LM.mdl')
        return model, lossval, lossval_dbg, relative_error
