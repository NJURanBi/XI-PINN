...
# ------------------ 加载模型并预测网格解 ------------------
model_p = Vanilla_Net(3, 23, 1, 2).double()
model_n = Vanilla_Net(3, 23, 1, 2).double()
model_p.load_state_dict(torch.load('best_model_p_LM_X-PINN_(3,23).mdl'))
model_n.load_state_dict(torch.load('best_model_n_LM_X-PINN_(3,23).mdl'))
model_p.eval()
model_n.eval()
...
plt.suptitle(f'X-PINN prediction and error at $t={t_val}$', fontsize=15)
