...
def optimize_parameters_LM(data_op, data_on, data_b, data_inip, data_inin, data_if, beta_p, beta_n, model_p, model_n, tr_iter_max, mu, lossval, lossval_dbg, relative_error, mu_div, mu_mul, device):
    ...
    def res_interface_N(vec, point, nor):
        ...
        n1, n2 = nor[0], nor[1]
        flux_p = beta_p * (dupdx * n1 + dupdy * n2)
        flux_n = beta_n * (dundx * n1 + dundy * n2)
        return flux_p - flux_n
    ...
    NL = [total_res, Nop, Non, Nb, Ninip, Ninin, Nif, Nif]
    ...
    try:
        while (lossval[-1] > tol_main) and (step <= tr_iter_max):
            ...
            if Comput_old: # need to compute loss_old and J_olds
                ...
                Lif_D = torch.vmap(res_interface_D, (None, 0))(all_params, data_if[0]).flatten().detach()
                Lif_N = torch.vmap(res_interface_N, (None, 0, 0))(all_params, data_if[0], data_if[1]).flatten().detach()
            ...
            if criterion:
                ...
                Jif_D = torch.vmap(torch.func.jacrev(res_interface_D, argnums=0), (None, 0))(all_params, data_if[0])
                Jif_D = Jif_D / NL_sqrt[6]
                Jif_N = torch.vmap(torch.func.jacrev(res_interface_N, argnums=0), (None, 0, 0))(all_params, data_if[0], data_if[1])
