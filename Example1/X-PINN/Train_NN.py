...
def train_neural_network(n_input, n_hidden, n_output, n_layers, num_o, num_b, num_ini, num_if, beta_p, beta_n, optimizer, device, n_runs):
    ...
    for run in range(n_runs):
        ...
        model_p = Vanilla_Net(n_input, n_hidden, n_output, n_layers).to(device)
        model_n = Vanilla_Net(n_input, n_hidden, n_output, n_layers).to(device)
        print("Total number of paramerters in networks is {}  ".format(2*sum(x.numel() for x in model_p.parameters())))
        data_op_tr, data_on_tr, data_b_tr, data_inip_tr, data_inin_tr, data_if_tr = generate_train_data(num_o, num_ini, num_b, num_if, beta_p, beta_n, device)

        # Optimizer parametes
        if optimizer == 'Adam':
            total_epochs = 50000
            lr = 1e-3
            omodel_p, model_n, lossval, lossval_dbg, relative_error = optimize_parameters_Adam(data_op_tr, data_on_tr, data_b_tr, data_inip_tr, data_inin_tr, data_if_tr, beta_p, beta_n, model_p, model_n, total_epochs, lr, lossval, lossval_dbg, relative_error, device)
        elif optimizer == 'SOAP':
            total_epochs = 10000
            lr = 1e-3
            model_p, model_n, lossval, lossval_dbg, relative_error = optimize_parameters_SOAP(data_op_tr, data_on_tr, data_b_tr, data_inip_tr, data_inin_tr, data_if_tr, beta_p, beta_n, model_p, model_n, total_epochs, lr, lossval, lossval_dbg, relative_error, device)
        elif optimizer == 'LM':
            total_epochs = 2000
            mu = 1e8
            mu_div = 3.
            mu_mul = 2.
            model_p, model_n, lossval, lossval_dbg, relative_error = optimize_parameters_LM(data_op_tr, data_on_tr, data_b_tr, data_inip_tr, data_inin_tr, data_if_tr, beta_p, beta_n, model_p, model_n, total_epochs, mu, lossval, lossval_dbg, relative_error, mu_div, mu_mul, device)
        else:
            ValueError('error!')
