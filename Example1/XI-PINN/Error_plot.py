...
# ------------------ 加载模型并预测网格解 ------------------
model = Vanilla_Net(4, 64, 1, 2).double()
model.load_state_dict(torch.load('best_model_XIPINN_LM_XI-PINN_(3,64).mdl'))
model.eval()
data_add = add_dimension(data)
data_add_tensor = torch.tensor(data_add).double()
with torch.no_grad():
...
# 预测解与梯度（需用自动微分）
mc_data_add = add_dimension(mc_data_tmp)  # (N, 4)
...
dphidx = get_dphi_dx(mc_data_tmp)
dphidy = get_dphi_dy(mc_data_add)
phi = level_set_function(mc_data_tmp)
