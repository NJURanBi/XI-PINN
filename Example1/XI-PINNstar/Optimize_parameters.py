...
def optimize_parameters_Adam(data_op, data_on, data_b, data_ini, data_if, beta_p, beta_n, model, epochs, lr, lossval, lossval_dbg, relative_error, device):
    ...
    model.load_state_dict(torch.load('best_model_XIPINN_Adam.mdl'))
    return model, lossval, lossval_dbg
...
def optimize_parameters_SOAP(data_op, data_on, data_b, data_ini, data_if,  beta_p, beta_n, model, epochs, lr, lossval, lossval_dbg, relative_error, device):
    def loss_omega_p(data):
        ...
        dudt = gradu[0][:, 2][:, None]
        dudz = gradu[0][:, 3][:, None]
        gradux = autograd.grad(outputs=dudx, inputs=data[0], grad_outputs=torch.ones_like(u), create_graph=True,
                               retain_graph=True)
        d2udx2 = gradux[0][:, 0][:, None]
        d2udxdz = gradux[0][:, 3][:, None]
        graduy = autograd.grad(outputs=dudy, inputs=data[0], grad_outputs=torch.ones_like(u), create_graph=True,
                               retain_graph=True)
        d2udy2 = graduy[0][:, 1][:, None]
        d2udydz = graduy[0][:, 3][:, None]
        d2udz2 = autograd.grad(outputs=dudz, inputs=data[0], grad_outputs=torch.ones_like(u), create_graph=True,
                               retain_graph=True)[0][:, 3][:, None]
        Ut = dudt + dudz * phit
        Uxx = d2udx2 + 2 * phix * d2udxdz + phix * phix * d2udz2 + phix2 * dudz
        Uyy = d2udy2 + 2 * phiy * d2udydz + phiy * phiy * d2udz2 + phiy2 * dudz
        res = Ut - beta_p * (Uxx + Uyy)
...
def optimize_parameters_LM(data_op, data_on, data_b, data_ini, data_if, beta_p, beta_n, model, tr_iter_max, mu, lossval, lossval_dbg, relative_error, mu_div, mu_mul, device):
    ...
    def res_omega_p(func_params, data, f, phix, phiy, phit, phix2, phiy2):
        phix = phix[0]
        phiy = phiy[0]
        phit = phit[0]
        phix2 = phix2[0]
    ...
    def res_omega_n(func_params, data, f, phix, phiy, phit, phix2, phiy2):
        phix = -phix[0]
        phiy = -phiy[0]
        phit = -phit[0]
        phix2 = -phix2[0]
    ...
    try:
        while (lossval[-1] > tol_main) and (step <= tr_iter_max):
            ...
            if (Comput_old == True):  # need to compute loss_old and J_old
                ### computation of loss
                Lop = torch.vmap((res_omega_p), (None, 0, 0, 0, 0, 0, 0, 0))(func_params, data_op[0], data_op[1], data_op[2], data_op[3], data_op[4], data_op[5], data_op[6]).flatten().detach()
                Lon = torch.vmap((res_omega_n), (None, 0, 0, 0, 0, 0, 0, 0))(func_params, data_on[0], data_on[1], data_on[2], data_on[3], data_on[4], data_on[5], data_on[6]).flatten().detach()
            ...
            if criterion:
                per_sample_grads = torch.vmap(torch.func.jacrev(res_omega_p), (None, 0, 0, 0, 0, 0, 0, 0))(func_params, data_op[0], data_op[1], data_op[2], data_op[3], data_op[4], data_op[5], data_op[6])
                cnt = 0
                ...
                per_sample_grads = torch.vmap(torch.func.jacrev(res_omega_n), (None, 0, 0, 0, 0, 0, 0, 0))(func_params, data_on[0], data_on[1], data_on[2], data_on[3], data_on[4], data_on[5], data_on[6])
                cnt = 0
                for g in per_sample_grads:
                    g = per_sample_grads[g].detach()
