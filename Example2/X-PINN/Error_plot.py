...
def get_u(data):
    ...
    return np.where(lf >= 1e-6, u_p, u_n)[:, None]
...
def get_u_true_and_grad(data):
    ...
    grad_x1 = np.where(lf >= 1e-6, grad_p_x1, grad_n_x1)
    grad_x2 = np.where(lf >= 1e-6, grad_p_x2, grad_n_x2)
    grad_x3 = np.where(lf >= 1e-6, grad_p_x3, grad_n_x3)

    return u, np.stack([grad_x1, grad_x2, grad_x3], axis=1)


def XPINN_output(data, model_p, model_n):
    x1 = data[:, 0]
    x2 = data[:, 1]
    x3 = data[:, 2]
    t = data[:, 3]
    F1 = (x1 * torch.cos(np.pi * t / 2) + x2 * torch.sin(np.pi * t / 2)) ** 2
    F2 = (-x1 * torch.sin(np.pi * t / 2) + x2 * torch.cos(np.pi * t / 2)) ** 2
    F3 = (x3 - 0.5 * t + 0.25) ** 2
    lf = F1 / a ** 2 + F2 / b ** 2 + F3 / c ** 2 - 1
    output_p = model_p(data)
    output_n = model_n(data)
    mask = (lf >= 1e-6).double().unsqueeze(1)
...
# ================= 2. 加载 X‑PINN 模型 =================
model_p = Vanilla_Net(4, 45, 1, 3).double()
model_n = Vanilla_Net(4, 45, 1, 3).double()
model_p.load_state_dict(torch.load('best_model_p_LM_4_45.mdl'))
model_n.load_state_dict(torch.load('best_model_n_LM_4_45.mdl'))
model_p.eval()
model_n.eval()
...
with torch.no_grad():
    u_pred_grid = XPINN_output(data_tensor, model_p, model_n).cpu().numpy()
...
# mc_data = np.hstack([mc_points, t_val * np.ones((N_mc, 1))])
...
u_pred_mc_tensor = XPINN_output(mc_input, model_p, model_n)
...
plt.suptitle(f'X‑PINN prediction on slice $z=0$ at $t={t_val}$', fontsize=15)
