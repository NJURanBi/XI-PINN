...
def add_dimension_p(data):
    return np.hstack((data, np.ones((len(data[:, 0]), 1))))

def add_dimension_n(data):
    return np.hstack((data, -np.ones((len(data[:, 0]), 1))))
...
def generate_train_data(num_o, num_ini, num_b, num_if, beta_p, beta_n, device):
    ...
    xif_p_add = add_dimension_p(xif)
    xif_n_add = add_dimension_n(xif)
    ...
    data_op_tr = (xop_add, f_p)
    data_on_tr = (xon_add, f_n)
    data_b_tr = (xb_add, g)
    data_ini_tr = (xini_add, u0)
    data_if_D_tr = (xif_p_add, xif_n_add, psi_D)
    data_if_N_tr = (xif_p_add, xif_n_add, psi_N, nv)
    return data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_D_tr, data_if_N_tr
