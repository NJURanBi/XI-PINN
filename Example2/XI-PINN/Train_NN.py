...
from Optimize_parameters import optimize_parameters_LM, optimize_parameters_SOAP
...
def train_neural_network(n_input, n_hidden, n_output, n_layers, num_o, num_b, num_ini, num_if, beta_p, beta_n, optimizer, device, n_runs):
    ...
    for run in range(n_runs):
        ...
        data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_D_tr, data_if_N_tr = generate_train_data(num_o, num_ini, num_b, num_if, beta_p, beta_n, device)
        # Optimizer parametes
        if optimizer == 'Adam':
            total_epochs = 50000
            lr = 1e-3
            ...
            total_epochs = 10000
            lr = 1e-3
            model, lossval, lossval_dbg, relative_error = optimize_parameters_SOAP(data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_D_tr, data_if_N_tr, beta_p, beta_n, model, total_epochs, lr, lossval, lossval_dbg, relative_error, device)
        elif optimizer == 'LM':
            total_epochs = 2000
            mu = 1e8
            mu_div = 3.
            mu_mul = 2.
            model, lossval, lossval_dbg, relative_error = optimize_parameters_LM(data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_D_tr, data_if_N_tr, beta_p, beta_n, model, total_epochs, mu, lossval, lossval_dbg, relative_error, mu_div, mu_mul, device)
        else:
            ValueError('error!')
