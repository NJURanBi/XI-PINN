...
def add_dimension(data):
    ...
    return np.hstack((data, np.abs(lf)))
...
def get_dphi_dx1(data):
    phi_x1, _ = get_phi_grad(data)
    return phi_x1

def get_dphi_dx2(data):
    _, phi_x2 = get_phi_grad(data)
    return phi_x2
...
# ------------------ 模型加载 ------------------
model = Vanilla_Net(4, 64, 2, 3).double()
model.load_state_dict(torch.load('best_V_model_XIPINN_LM_large.mdl'))
model.eval()
data_add_grid = add_dimension(data_grid)
data_add_tensor = torch.tensor(data_add_grid).double()
with torch.no_grad():
    pred_in = model(data_add_tensor).cpu().numpy()
...
mc_add = add_dimension(mc_data)
mc_input = torch.tensor(mc_add, requires_grad=True, dtype=torch.float64)
pred_mc = model(mc_input)

grad1_pred = torch.autograd.grad(pred_mc[:,0].sum(), mc_input, create_graph=False, retain_graph=True)[0].detach().cpu().numpy()
grad2_pred = torch.autograd.grad(pred_mc[:,1].sum(), mc_input, create_graph=False, retain_graph=False)[0].detach().cpu().numpy()
u1_pred = pred_mc[:,0].detach().cpu().numpy()
u2_pred = pred_mc[:,1].detach().cpu().numpy()

sign = np.where(level_set_function(mc_data)[:,0] > 0, 1.0, -1.0)
dphi1 = get_dphi_dx1(mc_data).flatten()
dphi2 = get_dphi_dx2(mc_data).flatten()
...
def plot_modulus(mod_pred_2d, error_2d, mod_true_2d, cross_direction='x'):
    ...
    if cross_direction == 'x':
        idx = np.argmin(np.abs(x1 - 0.0))
        line_x = np.where(mask_circle[:, idx], x2, np.nan)
        ...
        x_label = '$y$'
        fixed_str = '$x = 0$'
        valid_mask = ~np.isnan(lf_line)
        interface_idx = np.arange(len(lf_line))[valid_mask][np.nanargmin(np.abs(lf_line[valid_mask]))]
        interface_pos = x2[interface_idx]
    else:  # 'y'
    ...
    con_left = mpatches.ConnectionPatch(
        ...
        axesA=inset_ax, axesB=axes[2], color="0.4", linewidth=1.2, linestyle='-')
    ...
    fig.suptitle(r'XI-PINN prediction of $|(u,v)^T|$ at $t=%.1f$' % t_val, fontsize=15)
    fig.savefig('pred_error_modulus.jpg', dpi=300)
