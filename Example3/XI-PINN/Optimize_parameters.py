...
def error_compute(model, func_params, data):
    ...
    u_true_0 = data[1].reshape(-1, 1) if data[1].ndim == 1 else data[1]
    u_true_1 = data[2].reshape(-1, 1) if data[2].ndim == 1 else data[2]
    u_true = torch.cat([u_true_0, u_true_1], dim=1)

    relative_l2 = torch.norm(u_numerical - u_true) / torch.norm(u_true)
    return relative_l2
