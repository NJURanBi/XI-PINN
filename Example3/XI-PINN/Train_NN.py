...
def train_neural_network(V_n_input, V_n_hidden, V_n_output, V_n_layers, P_n_input, P_n_hidden, P_n_output, P_n_layers, num_o, num_b, num_ini, num_if, nu_p, nu_n, optimizer, device, n_runs):
    ...
    for run in range(n_runs):
        ...
        V_model = Vanilla_Net(V_n_input, V_n_hidden, V_n_output, V_n_layers).to(device)
        P_model = Vanilla_Net(P_n_input, P_n_hidden, P_n_output, P_n_layers).to(device)
        print("Total number of paramerters in networks is {}  ".format(sum(x.numel() for x in V_model.parameters()) + sum(x.numel() for x in P_model.parameters())))
        data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_tr = generate_train_data(num_o, num_ini, num_b, num_if, nu_p, nu_n, device)
        # Optimizer parametes
        if optimizer == 'Adam':
            total_epochs = 50000
            lr = 1e-3
            ...
            total_epochs = 10000
            lr = 1e-3
            optimize_parameters_SOAP(data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_tr, nu_p, nu_n, V_model, P_model, total_epochs, lr, lossval, lossval_dbg, relative_error, device)
        elif optimizer == 'LM':
            total_epochs = 2000
            mu = 1e8
            mu_div = 3.
            mu_mul = 2.
            optimize_parameters_LM(data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_tr, nu_p, nu_n, V_model, P_model, total_epochs, mu, lossval, lossval_dbg, relative_error, mu_div, mu_mul, device)
        else:
            ValueError('error!')
