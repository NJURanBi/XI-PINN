...
def get_phi(data):
    ...
    phi = (xi**2 + eta**2) - 0.4 - (1.0 / np.pi) * torch.sin(torch.pi * xi) * torch.cos(torch.pi * eta)
    return phi[:, None]
...
def get_phi_grad(data):
    ...
    dxi_dx1 = torch.cos(t); deta_dx1 = -torch.sin(t)
    dxi_dx2 = torch.sin(t); deta_dx2 = torch.cos(t)
...
def get_phi_hessian(data):
    ...
    xi = x1*torch.cos(t) + x2*torch.sin(t)
    eta = -x1*torch.sin(t) + x2*torch.cos(t)
    phi_xixi = 2 + torch.pi * torch.sin(torch.pi*xi) * torch.cos(torch.pi*eta)
    phi_xieta = torch.pi * torch.cos(torch.pi*xi) * torch.sin(torch.pi*eta)
    phi_etaeta = 2 + torch.pi * torch.sin(torch.pi*xi) * torch.cos(torch.pi*eta)  # 与 phi_xixi 相同
...
def get_u(data, model):
    ...
    mask = phi > 0
    u1 = torch.where(mask, u_p1, u_n1)
    u2 = torch.where(mask, u_p2, u_n2)
    return u1[:, None], u2[:, None]

# 真解及其梯度（解析，已校正）
def get_u_true_and_grad(data, model):
    x1 = data[:,0]; x2 = data[:,1]; t = data[:,2]
    u_base1 = torch.exp(x1)*torch.sin(x2+t)
    u_base2 = torch.exp(x1)*torch.cos(x2+t)
    du_base1_x1 = u_base1
    du_base1_x2 = torch.exp(x1)*torch.cos(x2+t)
    du_base2_x1 = u_base2
    du_base2_x2 = -torch.exp(x1)*torch.sin(x2+t)
    ...
    mask = phi > 0
    u1 = torch.where(mask, u_base1 + phi*phi_x2, u_base1)
    u2 = torch.where(mask, u_base2 - phi*phi_x1, u_base2)
    grad1_x1 = torch.where(mask, du_p1_x1, du_n1_x1)
    grad1_x2 = torch.where(mask, du_p1_x2, du_n1_x2)
    grad2_x1 = torch.where(mask, du_p2_x1, du_n2_x1)
    grad2_x2 = torch.where(mask, du_p2_x2, du_n2_x2)

    grad1 = torch.stack([grad1_x1, grad1_x2], axis=1)
    grad2 = torch.stack([grad2_x1, grad2_x2], axis=1)
...
model = Vanilla_Net(4, 32, 2, 2).double()
model.load_state_dict(torch.load('best_V_model_XIPINN_LM_small.mdl'))
model.eval()
# ------------------ 网格上的真解（仅圆内） ------------------
data_grid = torch.tensor(data_grid).double()
u_true1_in, u_true2_in = get_u(data_grid, inverse_mapping)
u_true1_in = u_true1_in.cpu().detach().numpy()
u_true2_in = u_true2_in.cpu().detach().numpy()
lf_in = level_set_function(data_grid, inverse_mapping).cpu().detach().numpy()
data_add_grid = add_dimension(data_grid, inverse_mapping)
with torch.no_grad():
    pred_in = model(data_add_grid).cpu().detach().numpy()
...
mc_data = torch.tensor(mc_data).double()
# 真解与解析梯度
u1_true, u2_true, grad1_true, grad2_true = get_u_true_and_grad(mc_data, inverse_mapping)
u1_true = u1_true.cpu().detach().numpy()
u2_true = u2_true.cpu().detach().numpy()
grad1_true = grad1_true.cpu().detach().numpy()
grad2_true = grad2_true.cpu().detach().numpy()
# 预测及梯度
mc_add = add_dimension(mc_data, inverse_mapping)
mc_input = mc_add.double().requires_grad_()
pred_mc = model(mc_input)

grad1_pred = torch.autograd.grad(pred_mc[:,0].sum(), mc_input,
                                 create_graph=False, retain_graph=True)[0].detach().cpu().numpy()
grad2_pred = torch.autograd.grad(pred_mc[:,1].sum(), mc_input,
...
# 梯度修正
sign = torch.where(level_set_function(mc_data, inverse_mapping)[:,0] > 0, 1.0, -1.0)
dphi1, dphi2, _, _, _ = compute_phi_derivative(mc_data, inverse_mapping)
sign = sign.cpu().detach().numpy()
dphi1 = dphi1[:, 0].cpu().detach().numpy()
dphi2 = dphi2[:, 0].cpu().detach().numpy()
grad1_x1 = grad1_pred[:,0] + grad1_pred[:,3] * sign * dphi1
grad1_x2 = grad1_pred[:,1] + grad1_pred[:,3] * sign * dphi2
grad2_x1 = grad2_pred[:,0] + grad2_pred[:,3] * sign * dphi1
grad2_x2 = grad2_pred[:,1] + grad2_pred[:,3] * sign * dphi2
grad1_pred_corr = np.stack([grad1_x1, grad1_x2], axis=1)
grad2_pred_corr = np.stack([grad2_x1, grad2_x2], axis=1)
...
print(f"模长 |(u,v)| - 相对 L2 误差 : {rel_L2_mod:.6e}")
...
def plot_modulus(mod_pred_2d, error_2d, mod_true_2d):
    ...
    axes[0].set_title(r'Numerical $|(u,v)|$', fontsize=12)
    ...
    axes[2].set_xlabel(x_label); axes[2].set_ylabel(r'$|(u,v)|$')
    ...
    fig.suptitle(r'XI-PINN$^*$ prediction of $|(u,v)|$ at $t=%.1f$' % t_val, fontsize=15)
    fig.savefig('pred_error_modulus_XIPINN_star.jpg', dpi=300)

# 绘制模长图像（截面 y=0）
plot_modulus(mod_pred_2d, error_mod_2d, mod_true_2d)
