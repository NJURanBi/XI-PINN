...
def train_neural_network(n_input, n_hidden, n_output, n_layers, num_x, t_ini, t_end, tau, delta, optimizer, device):
    ...
    while (T < t_end+1e-6) and (detJ >= delta):
        print(f"\n===== Time: {T:.3f}s =====")
        data_tr = generate_train_data(num_x, T, t_ini, data_tr, data_tr_empty, device)
        data_tr_empty = False
        # Optimizer parametes
        if optimizer == 'Adam':
            total_epochs = 50000
