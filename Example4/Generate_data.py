...
X_mapping.load_state_dict(torch.load('best_model_1e-15.mdl'))
inverse_mapping_1.load_state_dict(torch.load('best_model_0_0.2.mdl'))
inverse_mapping_2.load_state_dict(torch.load('best_model_0.2_0.4.mdl'))
inverse_mapping_3.load_state_dict(torch.load('best_model_0.4_0.6.mdl'))
inverse_mapping_4.load_state_dict(torch.load('best_model_0.6_0.8.mdl'))
inverse_mapping_5.load_state_dict(torch.load('best_model_0.8_1.mdl'))


def get_omega_points(N_op, N_on, N_t):
    t = np.linspace(0, 1, N_t)[:, None]
    x = np.random.uniform(low=0, high=1, size=30*N_on)[:, None]
    y = np.random.uniform(low=0, high=1, size=30*N_on)[:, None]
...
def get_interface_points(N_if, N_t):
    ...
    c_x = 0.5
    c_y = 0.75
    x = c_x + 0.15 * np.cos(theta)
    y = c_y + 0.15 * np.sin(theta)
    data = np.hstack((x, y, t[0] * np.ones_like(x)))
    data = torch.tensor(data, requires_grad=True)
...
def get_boundary_points(N_b):
    index = np.random.uniform(low=0, high=1, size=N_b)[:, None]
    ...
    t = np.random.uniform(low=0, high=1, size=4 * N_b)[:, None]
    data = np.hstack((xb, t, np.ones((4 * N_b, 1))))
    return data


def get_initial_points(N_ini):
    x = np.random.uniform(low=0, high=1, size=N_ini)[:, None]
    y = np.random.uniform(low=0, high=1, size=N_ini)[:, None]
