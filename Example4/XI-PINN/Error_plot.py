...
def add_dimension(data):
    data_tensor = torch.tensor(data, dtype=torch.float64)
    phi = level_set_function(data_tensor, flow_models).detach().cpu().numpy()
    sign = np.where(phi >= 0, 1.0, -1.0)
    return np.hstack([data, sign])

# ================== 加载 XI‑PINN 模型 ==================
xipinn_model_path = 'best_model_XIPINN_LM_medium.mdl'
model = Vanilla_Net(4, 64, 3, 2).double()
model.load_state_dict(torch.load(xipinn_model_path, map_location='cpu'))
model.eval()
...
for data_tmp in grid_datas:
    ...
    data_add = add_dimension(data_tmp)
    inp = torch.tensor(data_add, dtype=torch.float64)
    with torch.no_grad():
    ...
    data_tensor = torch.tensor(data_tmp, dtype=torch.float64)
    lf = level_set_function(data_tensor, flow_models).detach().cpu().numpy()
...
grad_pred_u_all = torch.autograd.grad(
    pred_mc[:, 0].sum(), mc_input,
    create_graph=False, retain_graph=True
)[0].detach().cpu().numpy()
grad_pred_v_all = torch.autograd.grad(
    pred_mc[:, 1].sum(), mc_input,
    create_graph=False, retain_graph=False
)[0].detach().cpu().numpy()

phi = level_set_function(torch.tensor(mc_data, dtype=torch.float64), flow_models)
phi = phi.detach().cpu().numpy()[:, 0]
sign_phi = np.where(phi >= 0, 1.0, -1.0)

grad_u_pred_final = grad_pred_u_all[:, :2].copy()
grad_v_pred_final = grad_pred_v_all[:, :2].copy()
...
rel_L2_u, rel_H1_u = rel_error_components(pred_mc[:, 0].detach().cpu().numpy(), u_true,
                                           grad_u_pred_final, grad_u_true)
rel_L2_v, rel_H1_v = rel_error_components(pred_mc[:, 1].detach().cpu().numpy(), v_true,
                                           grad_v_pred_final, grad_v_true)

# 模长误差
mod_true_mc = np.sqrt(u_true**2 + v_true**2)
mod_pred_mc = np.sqrt(pred_mc[:, 0].detach().cpu().numpy()**2 + pred_mc[:, 1].detach().cpu().numpy()**2)
...
def plot_modulus(pred_fields, error_fields, lf_fields):
    ...
    fig.suptitle(r'XI-PINN prediction and error of $|(u,v)^T|$', fontsize=18,
                 x=0.4, y=0.95)
    return fig

fig_mod = plot_modulus(mod_pred_fields, mod_error_fields, lf_fields)
fig_mod.savefig('pred_error_modulus.jpg', dpi=300)
plt.show()