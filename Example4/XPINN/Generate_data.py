...
def ini_points(num, models, device):
    x1 = np.random.uniform(low=0, high=1, size=num)[:, None]
    x2 = np.random.uniform(low=0, high=1, size=num)[:, None]
    t = np.zeros_like(x1)
    data = np.hstack((x1, x2, t))
...
def generate_train_data(num_o, num_ini, num_b, num_if, nu_p, nu_n, device):
    ...
    xinip, xinin = ini_points(num_ini, models, device)
    ...
    up0_1, up0_2 = get_u_p(xinip)
    un0_1, un0_2 = get_u_n(xinin)
    ...
    up0_1 = To_tensor(up0_1[:, None], device)
    up0_2 = To_tensor(up0_2[:, None], device)
    un0_1 = To_tensor(un0_1[:, None], device)
    un0_2 = To_tensor(un0_2[:, None], device)

    data_op_tr = (xop, f_p_1, f_p_2, v_p_1, v_p_2)
    data_on_tr = (xon, f_n_1, f_n_2, v_n_1, v_n_2)
    data_b_tr = (xb, g_1, g_2)
    data_inip_tr = (xinip, up0_1, up0_2)
    data_inin_tr = (xinin, un0_1, un0_2)
    data_if_tr = (xif, nor, phi_1, phi_2, psi_1, psi_2)
...
def generate_test_data(num_test, device):
    ...
    up1 = To_tensor(up1[:, None], device)
    up2 = To_tensor(up2[:, None], device)
    un1 = To_tensor(un1[:, None], device)
    un2 = To_tensor(un2[:, None], device)
