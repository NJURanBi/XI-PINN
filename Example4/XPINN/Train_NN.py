...
def train_neural_network(n_input, n_hidden, n_output, n_layers, num_o, num_b, num_ini, num_if, nu_p, nu_n, optimizer, device, n_runs):
    master_seed = 2026
    ...
    for run in range(n_runs):
        ...
        if optimizer == 'Adam':
            ...
            total_epochs = 10000
            lr = 1e-3
            optimize_parameters_SOAP(data_op_tr, data_on_tr, data_b_tr, data_inip_tr, data_inin_tr, data_if_tr, nu_p, nu_n, model_p, model_n, total_epochs, lr, lossval, lossval_dbg, relative_error, device)
        elif optimizer == 'LM':
            total_epochs = 2000
            mu = 1e8
            mu_div = 3.
            mu_mul = 2.
            optimize_parameters_LM(data_op_tr, data_on_tr, data_b_tr, data_inip_tr, data_inin_tr, data_if_tr, nu_p, nu_n, model_p, model_n, total_epochs, mu, lossval, lossval_dbg, relative_error, mu_div, mu_mul, device)
        else:
            ValueError('error!')
