...
# ============================================
# 1. 数据导入——只有 X-PINN 和 XI-PINN，各 small/large
# ============================================
cases = [
    # X-PINN
    ('X-PINN',   'small', '3', '23', 'loss_data_XPINN_small.npz',   'relative_errors_data_XPINN_small.npz'),
    ('X-PINN',   'large', '3', '45', 'loss_data_XPINN_large.npz',   'relative_errors_data_XPINN_large.npz'),
    # XI-PINN
    ('XI-PINN',  'small', '3', '32', 'loss_data_XIPINN_small.npz',  'relative_errors_data_XIPINN_small.npz'),
    ('XI-PINN',  'large', '3', '64', 'loss_data_XIPINN_large.npz',  'relative_errors_data_XIPINN_large.npz'),
]

num_epochs = 2001
epochs_all = np.arange(num_epochs)
epochs = epochs_all[::10]      # 每隔10个点采样，共201个点
data = {'epoch': epochs}
...
# 为 small/large 生成深浅颜色
color_map = {}
for method, base in method_colors.items():
    color_map[(method, 'small')] = lighten_color(base, factor=0.5)   # 浅
    color_map[(method, 'large')] = base                              # 深（原色）

# 线型：small 虚线，large 实线
linestyle_map = {
    'small': '--',
    'large': '-'
}
