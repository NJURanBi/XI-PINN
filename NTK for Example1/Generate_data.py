...
def generate_train_data(num_o, num_ini, num_b, num_if, beta_p, beta_n, device):
    ...
    eps = 1e-6
    xif_Van = xif[:, :2] + eps * nor
    xif_Van = np.hstack((xif_Van, xif[:, 2][:, None]))
    ...
    xif_Van = To_tensor_grad(xif_Van, device)
    ...
    data_if_Van_tr = (xif_add, xif_Van, nor)

    return data_op_tr, data_on_tr, data_b_tr, data_ini_tr, data_if_tr, data_if_Van_tr
